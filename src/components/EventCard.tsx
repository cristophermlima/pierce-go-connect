...
export default function EventCard({ id, title, location, date, image, category }: EventCardProps) {
  ...
  return (
    <Link 
      to={`/eventos/${id}`} 
      className="group glass-card rounded-2xl overflow-hidden transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-primary/20"
    >
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs px-2 py-1 rounded-full bg-primary/20 text-primary">Saiba mais</span>
        </div>
      </div>
    </Link>
  );
